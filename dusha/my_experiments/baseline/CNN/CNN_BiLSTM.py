...
class EmotionCNNBiLSTM(nn.Module):
    ...
    def _downsample_lengths(lengths: torch.Tensor) -> torch.Tensor:
        # Два MaxPool2d(kernel=2, stride=2) по временной оси => примерно floor(T/4)
    ...
    def forward(self, x, lengths):
        ...
        feats = feats.permute(0, 2, 1).contiguous()  # [B, T', C]
        ...
        # Берем последний валидный timestep по каждому объекту
...
def train_cnn_bilstm(
    ...
    print(
        f"Эпох: {epochs}, batch_size: {batch_size}, lr: {lr}, weight_decay: {weight_decay}, "
...
def main():
    parser = argparse.ArgumentParser(
        description="CNN + BiLSTM baseline для классификации эмоций на combine_balanced_*_small."
    )
    parser.add_argument(
        "--aggregated-dir",
        type=Path,
    ...
    parser.add_argument("--lstm-hidden-size", type=int, default=128, help="Hidden size LSTM.")
    parser.add_argument("--lstm-layers", type=int, default=2, help="Количество LSTM слоёв.")
    parser.add_argument(
        "--lstm-dropout",
        type=float,
        default=0.2,
        help="Dropout в LSTM (используется при lstm_layers > 1).",
    )
    parser.add_argument(
        "--unidirectional",
        action="store_true",
        help="Отключить bidirectional режим (по умолчанию BiLSTM).",
    )
    parser.add_argument(
        "--device",
        type=str,

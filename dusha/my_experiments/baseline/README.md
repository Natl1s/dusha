...
```bash
poetry run python dusha/my_experiments/baseline/logictic_regressoin.py
```

### Обучить новую модель
Всегда обучает модель заново (перезаписывает существующую):
```bash
poetry run python dusha/my_experiments/baseline/logictic_regressoin.py --mode train
```

### Загрузить существующую модель
Только загружает и оценивает существующую модель:
```bash
poetry run python dusha/my_experiments/baseline/logictic_regressoin.py --mode load
```

### Обучить без сохранения
Обучает модель, но не сохраняет результат:
```bash
poetry run python dusha/my_experiments/baseline/logictic_regressoin.py --mode train --no-save
```

## Сохранение модели

Модели сохраняются в `dusha/models_params/` с указанием датасета:
